...
(
  sleep 10  # Give system and network time to initialize
  ...
  if [[ "$CURRENT_SSID" != "$TARGET_SSID" ]]; then
    echo "⏩ Skipping authentication. Not connected to $TARGET_SSID (currently: $CURRENT_SSID)" >> "$LOG_FILE"
    exit 0
  fi
  ...
  # Prompt for credentials if file is missing
  if [[ ! -f "$CRED_FILE" ]]; then
    echo "⚠️ Credential file missing. Prompting user..." >> "$LOG_FILE"
  ...
  # Pre-authenticate with kinit to detect password expiry
  ...
  if [ $? -ne 0 ]; then
    ...
    if echo "$ERROR_MSG" | grep -qi "Password has expired"; then
      ...
      # Reset password with kpasswd
      sudo -u "$CURRENT_USER" kpasswd "$username@$REALM"

      # Prompt for new password to update file
      NEW_PASS=$(zenity --password --title="New Password" --text="Enter your new AD password:")
      ...
      if [ $? -ne 0 ]; then
        zenity --error --text="New password is incorrect. Cannot proceed."
        exit 1
      fi

      # Update password in credential file
    ...
    echo "✅ kinit authentication successful." >> "$LOG_FILE"
  fi
  ...
  # Create or update nmcli profile
  if ! nmcli --terse --fields NAME connection show | grep -Fxq "$USER_CON_NAME"; then
    ...
    echo "🔄 Updating existing Wi-Fi profile: $USER_CON_NAME" >> "$LOG_FILE"
  ...
  if nmcli -t -f NAME connection show --active | grep -q "$USER_CON_NAME"; then
    echo "🔌 Wi-Fi already active." >> "$LOG_FILE"
  else
    echo "▶️ Activating Wi-Fi: $USER_CON_NAME" >> "$LOG_FILE"
    nmcli connection up "$USER_CON_NAME" >> "$LOG_FILE" 2>&1
  fi

  echo "✅ Wi-Fi setup completed." >> "$LOG_FILE"
